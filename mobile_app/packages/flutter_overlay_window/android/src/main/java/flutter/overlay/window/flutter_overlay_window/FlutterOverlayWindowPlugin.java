...
public class FlutterOverlayWindowPlugin implements
    ...
    public void onAttachedToEngine(@NonNull FlutterPluginBinding flutterPluginBinding) {
        ...
        // PATCH (see PATCH.md): this plugin is attached twice — once to the
        // main app's engine, once to the overlay's own separately-created
        // engine (OverlayService.onCreate()'s FlutterEngineGroup). Both calls
        // used to unconditionally overwrite the static WindowSetup.messenger,
        // so whichever engine attached LAST (always the overlay, since it's
        // created after the main app starts) won. OverlayService then relays
        // every overlay->main shareData() call via WindowSetup.messenger.send(),
        // which — pointed at the overlay's own channel — delivered the message
        // right back to the overlay instead of the main app, and the main
        // app's overlayListener never received anything at all.
        if (!WindowSetup.mainMessengerSet) {
            WindowSetup.messenger = messenger;
            WindowSetup.messenger.setMessageHandler(this);
            WindowSetup.mainMessengerSet = true;
        }
    }
